...
|          构建乘积数组66          | 将将$B[i]$的计算拆分为$A[0]*...A[i-1]$和$A[i+1]*...A[n-1]$，即采用前缀积计算出当前位置$i$的左侧乘积和右侧乘积 |
...
| 二叉搜索树的最近公共祖先68Ⅰ | 判断$p$和$q$是否都在左子树或都在右子树或者在两侧，如果都在左/右子树，则往左/右子树遍历；如果在root树的两侧，则root就是最近公共祖先 |
...
|   把字符串转换成整数67   | 先去除两侧的空白部分，再判断首位是加号、减号还是无符号，再遍历字符串直到遇到非数字，结果的计算公式为$result = result * 10 + (c[j] - '0')$。难点在于判断结果是否越界 |
...
|     求1到n的累加和64     |                    $n += sumNums(n - 1)$                     |
|   不用加减乘除做加法65   | 两个数的当前二进制位为a和b，则将$a+b$转换为$n+c$，其中$\text{n=a⊕b c=a&b<<1}$ |
